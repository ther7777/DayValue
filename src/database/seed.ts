...
export async function seedSampleData(db: SQLiteDatabase): Promise<void> {
  ...
  await db.runAsync(
    `INSERT INTO Subscriptions (name, category, icon, billing_cycle, cycle_price, start_date, status)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ['iCloud+ 200GB (示例)', 'software', '💿', 'monthly', 21, '2023-08-01', 'active'],
  );
  ...
  await db.runAsync(
    ...
    ['美发储值卡 (示例)', 'beauty', '💇', 'amount', 800, 1000, 620, '2025-12-20', 30, 'active'],
  );
  ...
  await db.runAsync(
    ...
    ['私教计次卡 (示例)', 'fitness', '🏋️', 'count', 1200, 20, 15, '2026-02-25', 14, 'active'],
  );
